...
| **小红书数据** | 消费者舆情、产品反馈、趋势洞察 | 内置脚本。需配置 API Key：创建 `~/.alpha_insights.json` 并写入 `{"tikHubApiKey": "你的Key"}`（[注册 TikHub](https://tikhub.io) 获取） |

### 🟡 可选扩展（需要配置）

| 数据源 | 说明 | 所需配置 |
|--------|------|---------|
| **知识库** | 历史研究报告、行业笔记 | Notion MCP / Confluence MCP 等 |
