...
| 4→5 | `evidence_base.md` 存在且 ≥ 10 行；核心数据至少 1 条 ≥B 级 | B 级以上证据占比 < 50% |
| 5→6 | `insights.md` 存在且含评分 | 洞察数 < 3 |
| 6→7 | `report.html` 存在且 ≥ 5KB | 封面/目录/尾页缺失 |

**规则**：FAIL 条件不满足时，**禁止进入下一 Stage**，必须修复或回退。WARN 条件不满足时，告知用户后可继续。IQR 复核（Tier 2+）返回 BLOCK 等同于 FAIL，必须修复后重跑 IQR。
...
**输出**: `workspace/{project}/research_definition.md`，结构如下：
...
**🔍 IQR 复核**（Tier 2+ 时）：用户确认后、进入 Stage 3 前，加载 `resources/quality_review.md` 的 Stage 2 IQR 模板，启动独立 Subagent 评估研究定义质量。结果按 PASS/REVISE/BLOCK 处理。
...
**🔍 IQR 复核**（Tier 2+ 时）：进入 Stage 5 前，加载 `resources/quality_review.md` 的 Stage 4 IQR 模板，启动独立 Subagent 评估证据基础质量。重点关注证据覆盖度和置信度分布。
...
**档位控制**: Tier 1 规则 1→2→7 | Tier 2 规则 1→2→3→5→7→8 | Tier 3 全部 8 条
...
**执行**: 叙事弧线设计 → 逐章生成（每章自检 7 项，见 `report_standards.md`）→ 整合输出 → **🔍 IQR 复核**（Tier 2+ 时，加载 `resources/quality_review.md` Stage 6 IQR 模板，启动独立 Subagent 评估报告质量，按建议修正后再交付） → 交付包整理
...
| 6 | 读取 insights.md · Review Dashboard(Tier 2+) · Python 脚本生成 HTML · ECharts 用 dk 变量拼接 · 结论先行 · 证据可追溯 · 反模式自检 · 章节自检（按 report_standards.md 清单）· ≥3 ECharts 图表(Tier 2+/Tier 3 ≥6) · **IQR 复核**(Tier 2+) |
